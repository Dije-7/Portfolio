...
import type { Projects, DocumentTypes } from 'contentlayer/generated'
...
export function sortedProjectsPost(allProjects: Projects[]) {
  return allProjects.sort((a, b) => dateSortDesc(a.date, b.date)).filter((p) => p.draft === false)
}
...
export async function getAllTags(allProjects: Projects[]) {
  const tagCount: Record<string, number> = {}
  // Iterate through each post, putting all found tags into `tags`
  allProjects.forEach((file) => {
