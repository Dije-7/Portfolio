...
const generateRssItem = (post) => `
  <item>
    <guid>${siteMetadata.siteUrl}/Projects/${post.slug}</guid>
    <title>${escape(post.title)}</title>
    <link>${siteMetadata.siteUrl}/Projects/${post.slug}</link>
...
async function generate() {
  // RSS for Projects post
  if (allProjects.length > 0) {
    const rss = generateRss(allProjects)
